...
void Java_com_example_opengl_RelayNative_onMouseEvent(JNIEnv * env,
		jobject jobj, int mouseType, int x, int y) {
	MousePos pos;
	pos.X = x;
	pos.Y = y;
	LOGI_D("mouseType = %d, x = %d, y = %d",mouseType,x,y);
	g_ojbInputManager.MouseEven((MouseType) mouseType, pos);
}
